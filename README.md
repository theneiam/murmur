...
- Floating indicator with a live level meter while recording and a "Transcribing…" state until the text lands. Optionally (*Settings → General → Show floating status panel*, off by default) it stays on screen as a small draggable status pill showing that Murmur is running and which key to hold.
